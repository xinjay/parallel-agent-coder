...
## Step 2：读取任务表提取波次 (Wave Extraction)

对每个未完成任务：
1. 强制读取任务拆解表中的 `执行波次`（如 `Wave 1`, `Wave 2`）列。
2. 将任务按所属的波次进行自然分组。
3. （向后兼容）如果旧表没有该列，必须要求用户先手动在表格中补齐波次列，或者重新运行新版 `/task-decompose`，严禁主控在未授权的情况下自行瞎猜波次。

---

## Step 3：并行编排展示

主控**不再需要**自行推断和计算复杂的依赖树（DAG），直接依据表格中硬编码的波次，输出并行执行计划即可。
如果单个 Wave 内的任务数超过 4 个（受 Agent 并行能力限制），主控自动将其切分为 Wave Na、Wave Nb 两个顺延批次展示。
...
Wave 2（并行）:
  ├─ INT-C-002 控制器 (0.3d) [Mid-tier]
  └─ INT-C-003 显示层 (0.5d) [Mid-tier]

Wave 3（并行）:
  ├─ INT-C-005 世界地图集成 (0.3d) 
  └─ INT-C-006 城建集成 (0.3d) 

Wave 4（串行）:
  └─ INT-C-007 端到端验收 (0.2d)
...
请确认执行方案：
1. 按上述分波执行（推荐）
2. 调整分波（说明要移动的任务，或者你可以直接修改 Markdown 表格里的数值）
...
- **跳过某些任务**：如"跳过 008"
- **调整依赖**：直接修改 Markdown 表格里的 Wave 值或者口头通知主控调整。
